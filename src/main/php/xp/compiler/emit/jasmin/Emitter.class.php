...
class Emitter extends \xp\compiler\emit\Emitter {
  ...
  public function emitStaticMethodCall($b, $call) {
    ...
    foreach ($call->arguments as $i => $arg) {
      ...
      $b->append('aastore')->append("\n");
    }

    $b->append('invokestatic ')->append(strtr($ptr->name(), '.', '/'))->append('/');
    $b->append($call->name)->append('([Ljava/lang/Object;)V');
    $b->append("\n");
  ...
  public function emitArrayAccess($b, $access) {
    ...
    $b->append('aload_0')->append("\n");

    if ($access->offset instanceof \xp\compiler\ast\IntegerNode) {
      $b->append('iconst_'.$access->offset->resolve())->append("\n");
    }

    $b->append('aaload')->append("\n");
  ...
  protected function emitIf($b, $if) {
    ...
    // Condition
    $b->append('aload_0')->append("\n");     // XXX
    $b->append('arraylength')->append("\n"); // XXX
    $b->append('iconst_0')->append("\n");    // XXX
    $b->append('if_icmpgt ');
  ...
  protected function emitMethod($b, $method) {
    ...
    $b->append('.method ')->append(implode(' ', Modifiers::namesOf($method->modifiers)))->append(' ')->append($method->name);
